...
class DeliveryViewsTests(TestCase):
    ...
    def setUp(self):
        ...
        request = DeliveryRequest.objects.create(
            from_address='mabda', 
            to_address='maghsad',
            delivery_time=datetime(2023, 2, 22, 11, 30),
            created_by=self.user,
            cost=9.99,
        )
        self.delivery_data = {
            'request': request,
            'delivered_at': datetime(2023, 2, 25, 9, 0),
            'delivered_by_id': self.user.id
        }
        self.delivery = Delivery.objects.create(**self.delivery_data)

    def test_delivery_list_view(self):
        response = self.client.get(reverse_lazy('delivery-list'))
        self.assertEqual(response.status_code, 200)

    def test_delivery_detail_view(self):
        url = reverse_lazy('delivery-detail', args=[self.delivery.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_delivery_create_view(self):
        response = self.client.post(reverse_lazy('delivery-create'), self.delivery_data)
        self.assertEqual(response.status_code, 200) 
        self.assertEqual(Delivery.objects.count(), 1)

    def test_delivery_update_view(self):
        url = reverse_lazy('delivery-update', args=[self.delivery.id])
        data = self.delivery_data.copy()
        data['delivered_at'] = datetime(2023, 2, 25, 9, 0)
        
        response = self.client.post(url, data)
        self.delivery.refresh_from_db()
        self.assertEqual(self.delivery.delivered_at, datetime(2023, 2, 25, 9, 0))

    def test_delivery_delete_view(self):
        url = reverse_lazy('delivery-delete', args=[self.delivery.id])  
        response = self.client.post(url)     
        self.assertEqual(response.status_code, 302)
        self.assertFalse(Delivery.objects.filter(id=self.delivery.id).exists())
